...
5. From an existing cardforcoin checkout, run `cfc-local worktree create <ticket> --slug <slug>`. Do not create the branch or directory with separate Git commands. The command creates `~/work/cardforcoin-worktrees/<ticket>`, creates the branch from local `develop`, and runs setup.
